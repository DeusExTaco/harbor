...
test_platform_image() {
    ...
    case "$PLATFORM" in
        "linux/arm/v7")
            if [ -n "$memory_usage" ] && [ "$memory_usage" -gt 200 ] 2>/dev/null; then
            ...
            ;;
        "linux/arm64")
            if [ -n "$memory_usage" ] && [ "$memory_usage" -gt 256 ] 2>/dev/null; then
...
cleanup_all_containers() {
    ...
    for container in $(docker ps -a --format '{{.Names}}' | grep 'harbor-test-' 2>/dev/null || true); do
...
show_final_summary() {
    ...
    local running_containers=$(docker ps --format '{{.Names}}' | grep 'harbor-test-' 2>/dev/null || echo "")

    if [ -n "$running_containers" ]; then
        for container in $running_containers; do
            local port=$(docker port $container 8080/tcp 2>/dev/null | cut -d':' -f2 || echo "unknown")
...
main() {
    ...
    local test_dockerhub=false  # Default to GHCR only since we haven't set up Docker Hub yet
...
Options:
  --ghcr-only          Test only GHCR images (default)
  --dockerhub-only     Test only Docker Hub images
  --both               Test both GHCR and Docker Hub
...
Examples:
  $0                                    # Test GHCR images on all platforms
  $0 --both --cleanup-after            # Test both registries, cleanup after

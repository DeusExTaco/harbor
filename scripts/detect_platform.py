...
def create_platform_env_file(optimization_settings: Dict[str, Any]) -> None:
    ...
    with open(env_file, 'w') as f:
        ...
        f.write(f"MAX_CONCURRENT_UPDATES={optimization_settings['max_concurrent_updates']}\n")
        ...
        f.write(f"ENABLE_METRICS={str(optimization_settings['enable_metrics']).lower()}\n")
        f.write(f"REGISTRY_TIMEOUT={optimization_settings['registry_timeout']}\n")
        f.write(f"PULL_TIMEOUT_SECONDS={optimization_settings['pull_timeout_seconds']}\n")
        f.write(f"UPDATE_TIMEOUT_SECONDS={optimization_settings['update_timeout_seconds']}\n")
...
def display_platform_info() -> None:
    ...
    print(f"🔍 Harbor Platform Detection")
    print(f"============================")
    print(f"")
    ...
    print(f"⚙️ Applied Optimizations:")
    ...
    print(f"   Metrics: {'Enabled' if optimization_settings['enable_metrics'] else 'Disabled'}")
    ...
    if platform_info['platform_category'] == 'armv7':
        print(f"")
        print(f"🥧 Raspberry Pi 3 Recommendations:")
        print(f"   • Use the provided Raspberry Pi Docker Compose file")
        print(f"   • Ensure adequate SD card space (8GB+ recommended)")
        print(f"   • Monitor memory usage with 'docker stats'")
        print(f"   • Consider using Docker socket proxy for enhanced security")
        print(f"   • Schedule updates during low-usage periods")
    elif platform_info['platform_category'] == 'arm64':
        print(f"")
        print(f"🎯 ARM64 Recommendations:")
        print(f"   • Excellent performance on Raspberry Pi 4 (4GB+ RAM)")
        print(f"   • Native performance on Apple Silicon Macs")
        print(f"   • Consider enabling all features if resources allow")
        print(f"   • ARM64 cloud instances work great with Harbor")
    elif platform_info['platform_category'] == 'amd64':
        print(f"")
        print(f"🖥️ AMD64 Recommendations:")
        print(f"   • Full performance and feature set available")
        print(f"   • Consider using PostgreSQL for larger deployments")
        print(f"   • Enable monitoring and metrics for observability")
        print(f"   • Use Docker Compose with all optional services")
...
def check_system_resources() -> Dict[str, Any]:
    ...
    try:
        ...
        mem_available_kb = int([line for line in meminfo.split('\n') if 'MemAvailable:' in line][0].split()[1])
...
def display_resource_info() -> None:
    ...
    if resources['disk_free_gb'] < 2:
        ...
        print(f"   Consider reducing LOG_RETENTION_DAYS or CLEANUP_KEEP_IMAGES")
    ...
    if resources['mem_total_gb'] > 0:
        if resources['mem_total_gb'] < 1:
            ...
            print(f"   Harbor may struggle with default settings")
            print(f"   Consider using ARMv7 optimizations even on other platforms")
        elif resources['mem_total_gb'] < 2:
            print(f"💡 Note: Limited RAM ({resources['mem_total_gb']:.1f}GB total)")
            print(f"   ARM optimizations are recommended")
        else:
